...
class TestSummaryCareRecord(AsyncHTTPTestCase):
    ...
    def test_scr_adaptor_calls_mhs_end_point_happy_path(self, request_mock):
        ...
        @mock.patch('utilities.message_utilities.MessageUtilities.get_uuid')
        @mock.patch('comms.common_https.CommonHttps.make_request')
        def test_success_response_parsing(self, request_mock, uuid_mock):
            # Arrange
            expected_response = {
                "messageRef": "9C534C19-C587-4463-9AED-B76F715D3EA3",
                "messageId": "2E372546-229A-483F-9B11-EF46ABF3178C",
                "creationTime": "20190923112609",
                "messageDetail": "GP Summary upload successful"
            }

            uuid_mock.return_value = "yes-this-is-mocked"
            input_xml = FileUtilities.get_file_string(str(response_xml))
            request_mock.return_value = test_utilities.awaitable(input_xml)
            body = file_utilities.FileUtilities.get_file_dict(complete_data_path)
