...
from fake_spine.request_matching import MatcherAndResponse, RequestMatcher
...
def _match_message_id_in_ebxml(unique_identifier: str, message_id: str) -> RequestMatcher:
    # TODO: Decode the body once-per-request rather than once-per-matcher?
    # TODO: How is this 'unique_identifier' used? The value matters e.g. soap-fault-response-1 doesn't work
...
def component_test_responses():
    # TODO: these can be consts
    def soap_fault_response():
        return SpineResponse().override_response_code(500).override_response('soap_fault_single_error.xml')

    def ebxml_fault_response():
        return SpineResponse().override_response_code(500).override_response('ebxml_fault_single_error.xml')

    def async_reliable_success():
        return SpineResponse().override_response_code(202).override_response('async_reliable_success_response.xml')

    def retriable_soap_fault():
        return SpineResponse().override_response_code(500).override_response('soap_fault_that_should_be_retried.xml')

    response_mappings = []

    response_mappings.append(MatcherAndResponse(
