module "sdsapi_mock_ecs_service" {
  source = "../../modules/module_ecs_service"
  count           = var.gp2gp_create_sdsapi_mock ? 1 : 0
  project         = var.project
  component       = var.component
  environment     = var.environment
  region          = var.region
  module_instance = "gp2gp_sdsapi_mk_ecs"
  default_tags    = local.default_tags

  availability_zones = local.availability_zones

  image_name        = local.sdsapi_mock_image_name
  ...
  log_stream_prefix = var.gp2gp_sdsapi_mock_build_id
  ...
  command_variables     = local.sdsapi_mock_command_variables
