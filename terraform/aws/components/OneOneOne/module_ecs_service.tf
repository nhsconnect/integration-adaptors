module "OneOneOne_ecs_service" {
  ...
  container_port    = var.OneOneOne_nginx_container_port
  application_port  = var.OneOneOne_nginx_application_port
  launch_type       = var.OneOneOne_service_launch_type
  log_stream_prefix = var.OneOneOne_build_id
  healthcheck_path  = var.OneOneOne_nginx_healthcheck_path
  enable_load_balancing = true
  use_application_lb = false
  load_balancer_type = "network"
  protocol = "TCP"

  container_healthcheck_port = var.OneOneOne_nginx_healthcheck_port
