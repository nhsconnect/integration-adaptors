locals {
  ...
  image_name = "${var.account_id}.dkr.ecr.${var.region}.amazonaws.com/nhais-fake-responder:${var.nhais-responder_build_id}"
  # Use below when the ECR repo is created by terraform in account component.
  #image_name = "${data.terraform_remote_state.account.outputs.nhais_ecr_repo_url}:${var.nhais-responder_build_id}"
