resource "aws_security_group_rule" "additional_lb_from_incoming_cidr" {
  ...
  description = "Allow additional CIDRs to ${var.module_instance} Load Balancer in env: ${var.environment}"
}
...
resource "aws_security_group_rule" "additional_incoming_sg_to_lb" {
  ...
  to_port = var.application_port
  protocol = var.container_protocol
  description = "Allow additional SG: ${var.lb_allowed_security_groups[count.index]} to ${var.module_instance} Load Balancer in env: ${var.environment}"
}
...
resource "aws_security_group_rule" "additional_lb_from_incoming_sg" {
  ...
  to_port = var.application_port
  protocol = var.container_protocol
  description = "Allow from additional SG: ${var.lb_allowed_security_groups[count.index]} to ${var.module_instance} Load Balancer in env: ${var.environment}"
}
