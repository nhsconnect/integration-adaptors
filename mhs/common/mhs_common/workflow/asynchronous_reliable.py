...
class AsynchronousReliableWorkflow(common_asynchronous.CommonAsynchronousWorkflow):
    ...
    async def handle_outbound_message(self, message_id: str, correlation_id: str, interaction_details: dict,
        ...
        wdo = wd.create_new_work_description(self.persistence_store, message_id,
                                             wd.MessageStatus.OUTBOUND_MESSAGE_RECEIVED, workflow.ASYNC_RELIABLE)
        await wdo.publish()
        ...
        if response.code == 202:
            self._record_outbound_audit_log(workflow.ASYNC_RELIABLE, end_time, start_time, wd.MessageStatus.OUTBOUND_MESSAGE_ACKD)
            ...
            self._record_outbound_audit_log(workflow.ASYNC_RELIABLE, end_time, start_time, wd.MessageStatus.OUTBOUND_MESSAGE_NACKD)
            await wdo.set_status(wd.MessageStatus.OUTBOUND_MESSAGE_NACKD)
    ...
    async def handle_inbound_message(self, message_id: str, correlation_id: str, work_description: wd.WorkDescription,
                                     payload: str):
        logger.info('0009', 'handling inbound message')
