...
class TestCerts(unittest.TestCase):
    def test_create_certs_files_creates_folders(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            certs.create_certs_files(temp_dir)
    ...
    def test_create_certs_files_creates_private_key(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            private_key_file, = certs.create_certs_files(temp_dir, private_key=_TEST_FILE_CONTENTS)

            expected_private_key_filepath = pathlib.Path(temp_dir) / 'data' / 'certs' / 'client.key'
            self.assertEqual(str(expected_private_key_filepath), private_key_file)
    ...
    def test_create_certs_files_creates_local_cert(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            local_cert_file, = certs.create_certs_files(temp_dir, local_cert=_TEST_FILE_CONTENTS)

            expected_local_cert_filepath = pathlib.Path(temp_dir) / 'data' / 'certs' / 'client.pem'
            self.assertEqual(str(expected_local_cert_filepath), local_cert_file)
    ...
    def test_create_certs_files_creates_ca_certs(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ca_certs_file, = certs.create_certs_files(temp_dir, ca_certs=_TEST_FILE_CONTENTS)

            expected_ca_certs_filepath = pathlib.Path(temp_dir) / 'data' / 'certs' / 'ca_certs.pem'
            self.assertEqual(str(expected_ca_certs_filepath), ca_certs_file)
    ...
    def test_create_certs_files_creates_multiple_file(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            private_key_file, local_cert_file, ca_certs_file = certs.create_certs_files(temp_dir, private_key=test_file_contents_1, local_cert=test_file_contents_2, ca_certs=test_file_contents_3)
