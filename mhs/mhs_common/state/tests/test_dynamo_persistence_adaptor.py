...
class TestDynamoPersistenceAdaptor(unittest.TestCase):
    ...
    def setUp(self):
        ...
        patcher = unittest.mock.patch.object(state.dynamo_persistence_adaptor.aioboto3, "resource")
        self.mock_boto3_resource = patcher.start()
        self.addCleanup(patcher.stop)
        self.__configure_mocks(self.mock_boto3_resource)
        self.service = state.dynamo_persistence_adaptor.DynamoPersistenceAdaptor(
    ...
    async def test_add_io_exception(self):
        ...
        with self.assertRaises(state.dynamo_persistence_adaptor.RecordCreationError) as ex:
    ...
    async def test_get_io_exception(self):
        ...
        with self.assertRaises(state.dynamo_persistence_adaptor.RecordRetrievalError) as ex:
    ...
    async def test_delete_io_exception(self):
        ...
        with self.assertRaises(state.dynamo_persistence_adaptor.RecordDeletionError) as ex:
